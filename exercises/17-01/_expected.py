# -*- coding: utf-8 -*-
"""17-01 세기 시트의 기대값 — 모듈 수·프로시저 수·Option Explicit·On Error 쓴 모듈 수, 모듈별 주석 줄 비율.
...
쓰는 법: python -X utf8 exercises/17-01/_expected.py
"""
import json
import re
import sys
...
def expected_cells():
    mods = modules()
    procs = procedures(mods)
    code = {m: [ln for ln in lines if not is_comment(ln)] for m, lines in mods.items()}
    exp = {
        (SHEET, "B1"): len(mods),
        (SHEET, "B2"): len(procs),
        (SHEET, "B3"): sum(1 for m in mods if r2(code[m]) == "있음"),
        (SHEET, "B4"): sum(1 for m in mods if any(re.search(r"\bOn\s+Error\b", ln) for ln in code[m])),
    }
    for r, m in enumerate(sorted(mods), start=7):
        nonblank = [ln for ln in mods[m] if ln.strip()]
        exp[(SHEET, f"A{r}")] = m
        exp[(SHEET, f"B{r}")] = round(sum(is_comment(ln) for ln in nonblank) / len(nonblank), 6)
    return exp
