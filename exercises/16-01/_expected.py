# -*- coding: utf-8 -*-
"""16-01 매크로목록의 기대값 — 프로시저 이름을 가나다순으로 세웠을 때 줄마다 파일·프로시저·종류.
...
쓰는 법: python -X utf8 exercises/16-01/_expected.py
"""
import json
import re
import sys
...
def expected_cells():
    procs = by_name(procedures())
    exp = {}
    for r, (m, n, kind, prev, body) in enumerate(procs, start=2):
        exp[(SHEET, f"A{r}")] = f"{m}.bas"
        exp[(SHEET, f"B{r}")] = n
        exp[(SHEET, f"C{r}")] = kind
    exp[(SHEET, f"B{len(procs) + 2}")] = None      # 목록 다음 줄은 비어 있다
    return exp
